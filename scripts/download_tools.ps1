...
if (${env:ProgramFiles(x86)}) {
    $possibleLocations += Join-Path ${env:ProgramFiles(x86)} 'Tesseract-OCR'
}

$installedDir = $possibleLocations |
...
if (-not (Test-Path (Join-Path $tesseractDir 'tesseract.exe'))) {
    throw 'Could not copy Tesseract into the application bundle.'
}
